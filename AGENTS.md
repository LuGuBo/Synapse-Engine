...
- **No Persona Silos:** Personas are dynamically loaded JIT from `C:\AG SKILLS\` or global config based on `.agents/state.json` `"active_persona"`.
- **Pre-Execution Hook:** Query `ag_master_index` to fetch target domain skills before modifying any codebase resources.
- **Orchestrator Link:** Reference `C:\AG SKILLS\synapse-supreme-orchestrator\SKILL.md` to govern task routing.


## 2. Automatic UX/UI Aesthetic Protocol
- **Auto-Trigger:** Touches to HTML, CSS, JSX, Tailwind, layout, or badges automatically load `C:\AG SKILLS\ux-expert` and `frontend-design`.
